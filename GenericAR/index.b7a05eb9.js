...
function main() {
    ...
    if (hasId()) console.log(getId());
    else defaultBehavior();
}
main();

},{"./loadJSON":"8RRgo","./object/Model":"39wPA","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"8RRgo":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
function validatePoster(poster) {
    ...
    if (poster) {
        let res = true;
        for(let i = 0; i < poster.length && res; i++)res = autorizedExtensions.includes(fileExtension(poster[i]));
        return res;
    }
    return true;
}
function validateData(data) {
    return validateModelFile(data.modelName) && validateEnvironment(data.environment) && validatePoster(data.posters);
}
function loadJSON() {
    if ((0, _modelType.instanceOfModelType)(_cargoJson) && validateData(_cargoJson)) return _cargoJson;
    return null;
}
exports.default = loadJSON;

},{"../static/cargo/cargo.json":"gXTsN","./interface/modelType":"l53L2","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gXTsN":[function(require,module,exports) {
module.exports = JSON.parse('{"modelName":"cargo/ressources/cargo ship.glb","environment":"cargo/ressources/aircraft_workshop_01_1k.hdr","skybox":"true","posters":["cargo/ressources/cargo.webp"],"targets":[{"label":"H\xe9lice","type":"content","position":{"x":-0.42,"y":-0.1,"z":0.46},"normal":{"x":0,"y":0,"z":1},"content":"Avec cette h\xe9lice, le bateau peut avancer."},{"label":"Conteneurs","type":"content","position":{"x":0,"y":0.11,"z":0.45},"content":"Les conteneurs permettent de transporter de la marchandise"},{"type":"content","position":{"x":0.1,"y":-0.07,"z":0.48},"normal":{"x":0,"y":0,"z":1},"content":"La coque permet au bateau de ne pas couler"}]}');

},{}],"l53L2":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Model {
    constructor(json){
        ...
        json.targets.forEach((target)=>{
            this.annotations.push(new (0, _targetDefault.default)(new (0, _vector3Default.default)(target.position.x, target.position.y, target.position.z), target.normal ? new (0, _vector3Default.default)(target.normal.x, target.normal.y, target.normal.z) : null, target.label, target.type, target.content));
        });
        this.posters = json.posters;
    ...
    toHTML() {
        ...
        if (this.posters && this.posters.length > 1) this.multiplePosters();
        return html;
    }
    addModel() {
    ...
    addPoster() {
        if (this.posters && this.posters.length > 0) return `poster="${this.posters[0]}"`;
        return "";
    }
    addARButton() {
    ...
    multiplePosters() {
        ...
        let i = 0;
        setInterval(()=>togglePoster.setAttribute("poster", `${this.posters[++i % this.posters.length]}`), 2000);
    }
}
